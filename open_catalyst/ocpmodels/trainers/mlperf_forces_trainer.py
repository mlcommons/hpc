...
class MLPerfForcesTrainer(BaseTrainer):
    """
    Trainer class for the Structure to Energy & Force (S2EF) task for
    MLPerf HPC.
    ...
    def train(self):
        ...
        # Log general mlperf submission details
        if distutils.is_master():
            mllogger.event(key=mllog.constants.CACHE_CLEAR)
            mllogger.event(key=mllog.constants.SUBMISSION_BENCHMARK, value=self.config["task"]["mlperf_benchmark"])
            mllogger.event(key=mllog.constants.SUBMISSION_ORG, value=self.config["task"]["mlperf_org"])
            mllogger.event(key=mllog.constants.SUBMISSION_DIVISION, value=self.config["task"]["mlperf_division"])
            mllogger.event(key=mllog.constants.SUBMISSION_STATUS, value=self.config["task"]["mlperf_status"])
            mllogger.event(key=mllog.constants.SUBMISSION_PLATFORM, value=self.config["task"]["mlperf_platform"])
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                if iters % eval_every == 0:
                    if self.val_loader is not None:
                        ...
                        if (
                            "target_forces_mae" in self.config["task"]
